...
from KoNAMIC.paths import is_jean_zay_env, DatasetPaths


@dataclass
class SensorLoadSpec:
    num_steps_loaded: int | None = None
    num_traj_loaded: int | None = None
...
class SensorLoader:
    ...
    def load_raw_sensor_data(self) -> dict[str, dict]:
        return {
            split_name: self._load_split(
                ...
                num_traj_loaded=split_spec.num_traj_loaded,
            )
    ...
    def _load_split(
        ...
        *,
        ...
        return self._load_single_npz_dataset(
            ...
            num_traj_loaded=num_traj_loaded,
    ...
    def _load_single_npz_dataset(
        *,
        ...
        if num_traj_loaded is not None:
            if num_traj_loaded <= 0:
                raise ValueError(f"num_traj_loaded must be positive, got {num_traj_loaded}")
            if num_traj_loaded > x.shape[0]:
                raise ValueError(
                    f"Requested num_traj_loaded={num_traj_loaded}, but {npz_path} "
                    f"contains only {x.shape[0]} trajectories."
                )
            x = x[:num_traj_loaded]
            u = u[:num_traj_loaded]
            x_ref = x_ref[:num_traj_loaded]
