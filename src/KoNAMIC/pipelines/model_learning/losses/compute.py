...
from KoNAMIC.core.models.outputs.vision_outputs import ForwardOutputs, GroundTruth
...
class VisionLossComputer:
    ...
    def compute(
        ...
        if num_views == 2:
            ...
            loss_a_right = self.mse(
                pred.angles_right,
                targets.angles_right[:, target_slice],
            )
            loss_c = 0.5 * (loss_c_left + loss_c_right)
            loss_a = 0.5 * (loss_a_left + loss_a_right)
        else:
            loss_c = loss_c_left
            loss_a = loss_a_left

        # Diagnostics plus lisibles dans TensorBoard.
        # Convention issue des coordonnées centroid_diff:
        # left: x -> index 1, y -> index 3 dans votre ancien code.
        horizontal = self.mse(
            pred.centroids_left[..., 1],
            targets.centroids_left[:, target_slice, ..., 1],
        )
        vertical = horizontal # TODO: fix this line
        angle = loss_a
        ...
        if phases_active[0]:
            full_loss = full_loss + effective_weight(self.base["c"], "c") * loss_c
            full_loss = full_loss + effective_weight(self.base["a"], "a") * loss_a
        ...
        sub_losses: VisionSubLosses = {
            ...
            "angle": float(angle.detach().cpu().item()),
...
def mean_sub_losses(items: list[dict[str, float]]) -> dict[str, float]:
    if not items:
        return {}
