...
class KoopmanClosedLoopSimulator(ClosedLoopSimulator[ClosedLoopTrajectory]):
    def __init__(
        self,
        *,
        ...
        self.dt = eval_config.dt
        self.time = build_time_grid(eval_config.dt, eval_config.t_sim)
        self.num_steps = len(self.time)

    def run(self, scenario: Scenario) -> ClosedLoopTrajectory:
        x_init = np.asarray(scenario.x0, dtype=float).reshape(-1)
        self._set_controller_reference_from_scenario(scenario)

        return self._run_from_initial_state(x_init)

    def _run_from_initial_state(self, x_init: np.ndarray) -> ClosedLoopTrajectory:
        ...
        return build_closed_loop_trajectory(
            ...
            u_scaled=self._extract_u_scaled(self._extract_u_physical()),
            z_traj=z_traj,
    ...
    def _set_controller_reference_from_scenario(self, scenario: Scenario) -> None:
        reference = scenario.reference

        if isinstance(reference, tuple) and len(reference) == 3:
            state_ref_traj, im_ref_traj, z_ref_traj = reference

            self.controller.set_reference(z_ref_traj)
            self.controller.x_ref_traj = state_ref_traj
            self.controller.im_ref_traj = im_ref_traj
            return
    ...
    def _run_simulation_loop(self, x_init: np.ndarray):
        ...
        x_traj = np.zeros((self.num_steps, x_dim), dtype=float)
        im_traj = None
